...
变量定义
**********************************/
volatile _Bool flag_bujin_foreward=0;						//步进电机正转标志位
volatile _Bool flag_bujin_reversal=0;						//步进电机反转标志位
volatile _Bool flag_bujin_state=0;               //步进电机状态标志位
...
void STEP_TIM_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(STEP_TIM, TIM_IT_Update) != RESET)  						//检查定时器更新中断发生与否
	{
		...
		if(flag_bujin_foreward == 1)					//步进电机正转过程
		{
			...
			{
				motor_bujin_count = 0;
				if(motor_step < MOTOR_TARGET_STEP)
					motor_step++;
				else
				{
...
{
    flag_bujin_foreward = 0;
    motor_step = MOTOR_TARGET_STEP;          // 从当前位置开始重新计算步数
